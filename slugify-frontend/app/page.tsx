...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#080808] text-[#f0f0f0]">
      ...
      {/* NAV */}
      <nav className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-12 py-5 transition-all duration-300 ${scrolled ? 'bg-[#080808]/85 backdrop-blur-xl border-b border-[#1c1c1c]' : ''}`}>
      ...
      <section ref={heroRef} className="min-h-screen flex flex-col items-center justify-center text-center px-6 pt-28 pb-20 relative overflow-hidden">
        ...
        {/* glow */}
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[radial-gradient(circle,rgba(147,51,234,0.12)_0%,transparent_70%)] pointer-events-none" />
        ...
        <h1 className="animate-fade-up-1 font-display font-black text-[clamp(52px,8vw,96px)] leading-[0.95] tracking-[-3px] mb-6 relative z-10">
          Links that<br />
          <span className="text-gradient">mean something</span>
        </h1>

        <p className="animate-fade-up-2 text-[#666] text-lg max-w-md leading-relaxed mb-12 relative z-10">
        ...
        <div className={`animate-fade-up-3 flex items-center gap-0 bg-[#0f0f0f] border rounded-xl p-1.5 pl-4 w-full max-w-lg relative z-10 transition-all duration-200 ${url ? 'border-purple-500 shadow-[0_0_0_3px_rgba(147,51,234,0.15)]' : 'border-[#2e2e2e]'}`}>
          <input
            ...
            className="flex-1 bg-transparent outline-none text-[#f0f0f0] font-mono text-sm placeholder-[#444]"
          />
          <button
            onClick={handleSlugify}
            className={`px-5 py-2.5 rounded-lg font-display font-bold text-sm text-white transition-all ${slugified ? 'bg-green-600' : 'bg-purple-600 hover:bg-purple-700'}`}
          >
            {slugified ? '✓ Slugified!' : 'Slugify it →'}
          </button>
        </div>
        ...
        {/* stats */}
        <div className="animate-fade-up-4 flex w-full max-w-2xl mt-20 border-t border-b border-[#1c1c1c] relative z-10">
          {[
            ...
            <div key={i} className={`flex-1 py-6 text-center ${i < 3 ? 'border-r border-[#1c1c1c]' : ''}`}>
              <div className="font-display font-black text-3xl tracking-tight">
                {s.num.replace(/[0-9.]+/, m => `<span class="text-purple-500">${m}</span>`).includes('<span')
                  ? <span>{s.num.split(/([0-9.<>%]+)/).map((p, j) => /[0-9.<>%]+/.test(p) ? <span key={j} className="text-purple-500">{p}</span> : p)}</span>
                  : s.num}
              </div>
      ...
      {/* FEATURES */}
      <section id="features" className="max-w-5xl mx-auto px-12 py-32">
        <div className="reveal opacity-0 translate-y-8 transition-all duration-700">
          <p className="text-purple-400 font-mono text-xs uppercase tracking-widest mb-4">// what you get</p>
          <h2 className="font-display font-black text-[clamp(32px,4vw,52px)] tracking-tight leading-[1.05] mb-16">
            Everything a link<br /><span className="text-purple-500">deserves</span>
          </h2>
        </div>
        <div className="reveal opacity-0 translate-y-8 transition-all duration-700 delay-100 grid grid-cols-3 gap-px bg-[#1c1c1c] border border-[#1c1c1c] rounded-xl overflow-hidden">
          {features.map((f, i) => (
            <div key={i} className="bg-[#080808] hover:bg-[#0f0f0f] p-9 group transition-all relative overflow-hidden">
      ...
      {/* HOW IT WORKS */}
      <section id="how" className="bg-[#0f0f0f] border-t border-b border-[#1c1c1c] py-32">
        <div className="max-w-5xl mx-auto px-12">
          <div className="reveal opacity-0 translate-y-8 transition-all duration-700">
            <p className="text-purple-400 font-mono text-xs uppercase tracking-widest mb-4">// the flow</p>
            <h2 className="font-display font-black text-[clamp(32px,4vw,52px)] tracking-tight leading-[1.05] mb-20">
              Simple by design,<br /><span className="text-purple-500">powerful</span> underneath
            </h2>
          </div>
          <div className="reveal opacity-0 translate-y-8 transition-all duration-700 delay-100 grid grid-cols-4 gap-8 relative">
            <div className="absolute top-5 left-[calc(12.5%+16px)] right-[calc(12.5%+16px)] h-px bg-gradient-to-r from-purple-500 to-purple-500/20" />
            {steps.map((s, i) => (
              <div key={i}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-mono text-xs mb-5 relative z-10 ${i === 0 ? 'bg-purple-600 text-white shadow-[0_0_20px_rgba(147,51,234,0.4)]' : 'bg-[#080808] border border-[#2e2e2e] text-purple-400'}`}>
                  {s.num}
                </div>
      ...
      <section className="py-32 text-center relative overflow-hidden">
        ...
        <div className="reveal opacity-0 translate-y-8 transition-all duration-700 relative z-10">
          <h2 className="font-display font-black text-[clamp(40px,6vw,72px)] tracking-[-2px] leading-none mb-6">
          ...
          <div className="flex items-center justify-center gap-4">
            <Link href="/register" className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-3.5 rounded-lg font-display font-bold text-sm transition-all hover:-translate-y-px">
              Create your account →
            </Link>
            <Link href="#features" className="text-[#666] hover:text-[#f0f0f0] px-8 py-3.5 rounded-lg text-sm border border-[#2e2e2e] hover:border-[#666] transition-all">
              See features
            </Link>
          </div>
        </div>
      ...
      {/* FOOTER */}
      <footer className="border-t border-[#1c1c1c] px-12 py-10 flex items-center justify-between">
